#!/usr/bin/env python3
"""
profile_matmul_dataset.py
- Sweeps parameter combinations (including STRIDE_ACCESS)
- Compiles CUDA file per combo (nvcc --maxrregcount)
- Logs GPU power.draw for POWER_LOG_DURATION seconds and computes average
- Parses GFLOPS, MSEC_PER_MATMUL, THREADS_PER_BLOCK, STRIDE_ACCESS, RESULT
- Writes one row per configuration to CSV (includes exec time and power_log_duration)
"""
...
# Config
# -------------------
CUDA_SOURCE = "matmul_param_dataset.cu"
CUDA_BINARY = "matmul_param_dataset_bin"
LOG_DIR = Path("power_logs_all_configs")
CSV_FILE = Path("matmul_dataset_with_stride.csv")
...
MSEC_RE = re.compile(r"MSEC_PER_MATMUL:\s*([0-9]*\.?[0-9]+)")
...
CUDA_ERR_RE = re.compile(r"CUDA (?:launch|sync|event).*?:\s*(.*)")

def compile_cuda(block, grid, shared, unroll, reg, work, stride):
    print(f"[Compiling] BLOCK={block}, GRID={grid}, SHARED={shared}, UNROLL={unroll}, REG={reg}, WORK={work}, STRIDE={stride}")
    # compile with nvcc and max register count
    cmd = [
        "nvcc", "-O3",
        f"--maxrregcount={reg}",
        f"-DBLOCK_SIZE={block}",
...
def start_power_log(log_path):
    # query only power.draw (no timestamp to keep parsing simple)
    cmd = [
        "nvidia-smi",
...
def run_binary_and_profile(block, grid, shared, unroll, reg, work, stride):
    # start power logging
    log_name = f"power_b{block}_g{grid}_s{shared}_u{unroll}_r{reg}_w{work}_str{stride}.log"
    ...
    # if the program took longer than POWER_LOG_DURATION we still wait a bit less
    remaining = POWER_LOG_DURATION - (exec_time if exec_time < POWER_LOG_DURATION else 0)
    if remaining > 0:
        # sleep remaining seconds to gather full power window
...
def main():
    combos = list(product(BLOCK_SIZES, GRID_SCALES, SHARED, UNROLL, REGISTER_LIMITS, WORK_PER_THREAD, STRIDES))
    total = len(combos)
    ...
    with open(CSV_FILE, "a", newline="") as csvf:
        ...
        for idx, (b,g,s,u,r,w,stride) in enumerate(combos, start=1):
            print(f"\n=== Run {idx}/{total} ===")
            # quick sanity check: threads per block can't exceed 1024 on most devices.
            threads_pb = b * b
            if threads_pb > 1024:
                print(f"Skipping config: threads_per_block={threads_pb} exceeds 1024")
            ...
            ok, compile_out = compile_cuda(b,g,s,u,r,w,stride)
            if not ok:
                print("Compilation failed; writing row with compile error.")
                row = {k: None for k in header}
                row.update({"block_size":b,"grid_scale_x":g,"use_shared":s,"use_unroll":u,"register_limit":r,"work_per_thread":w,"stride_access":stride,"threads_per_block":threads_pb,"pass_fail":"COMPILE_FAIL","cuda_error_str":str(compile_out)})
                writer.writerow(row)
                csvf.flush()
                continue

            res = run_binary_and_profile(b,g,s,u,r,w,stride)
            # if binary failed (nonzero exit) but CUDA error string present, keep it for debugging
            if res["exit_code"] != 0:
                print(f"Binary exit code {res['exit_code']}. CUDA error: {res.get('cuda_error_str')}")
            else:
                print(f"PASS/FAIL: {res['pass_fail']} | GFLOPS: {res['gflops']} | Time(ms): {res['msec_per_matmul']} | Power(W): {res['avg_power_watts']}")

            # write row (keep formatting)
            writer.writerow({
            ...
            csvf.flush()
            # small delay to avoid hammering nvcc / GPU too quickly
            time.sleep(0.5)
