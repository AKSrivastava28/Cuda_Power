...
- Writes one row per configuration to CSV (includes exec time and
power_log_duration)
"""
...
CSV_FILE = Path("matmul_dataset_with_stride_corrected.csv")
...
def run_binary_and_profile():
    # Not used; we call run inline in run_and_profile
    pass

def run_and_profile(block, grid, shared, unroll, reg, work, stride):
    # start power logging
...
def main():
    ...
    with open(CSV_FILE, "a", newline="") as csvf:
        ...
        for idx, (b,g,s,u,r,w,stride) in enumerate(combos, start=1):
            ...
            ok, comp_out = compile_cuda(b,g,s,u,r,w,stride)
            if not ok:
            ...
            res = run_and_profile(b,g,s,u,r,w,stride)
            if res["exit_code"] != 0:
